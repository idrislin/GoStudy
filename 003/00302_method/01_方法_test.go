...
func (p Person) GetAgeNonPointer() int {
	return p.Age
}
...
func TestPerson(t *testing.T) {
	...
	log.Println(oldBrotherMa.GetAgeNonPointer())
}
...
/*
	...
	不管方法的接收器是指针类型还是非指针类型，
	都是可以通过指针/非指针类型进行调用的，编译器做类型转换。
*/

type Pointer *int

func (p *Pointer) GetPointer() {}
func (p Pointer) SetPointer()  {}

/*
	嵌入结构体扩展方法,类似 java的继承
*/
...
/*
	...
	封装有时候也被叫做信息隐藏，同时也是面向对象编程最关键的一个方面。

	Go 语言只有一种控制可见性的手段：
	大写首字母的标识符会从定义它们的包中被导出，小写字母的则不会。
	这种限制包内成员的方式同样适用于 struct 或者一个类型的方法。
	因而如果我们想要封装一个对象，我们必须将其定义为一个 struct

	封装提供了三方面的优点：
	首先，因为调用方不能直接修改对象的变量值，
	其只需要关注少量的语句并且只要弄懂少量变量的可能的值即可。

	第二，隐藏实现的细节，可以防止调用方依赖那些可能变化的具体实现，
	这样使设计包的程序员在不破坏对外的 api 情况下能得到更大的自由。
...
// 课堂作业：编写一个类型，用于增删改查键值对，并且提供获取所有键、值数组的方法
